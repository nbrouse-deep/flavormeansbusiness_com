...
var MSG_CANT_VOTE = "You have already voted! Please come back after midnight.";
...
CCookie.prototype =
{
	...
	del : function( name )
	{
		document.cookie = name + '=; expires=Mon, 6 Oct 2014 17:08:00 UTC; path=/';
	}
}
...
CAjaxPoll.prototype =
{
	...
	run : function()
	{
		...
		this.form.find( '.ajax-poll-btn-vote' ).click( function(e){
			...
			form.find( '.ajax-poll-item-bar' ).hide();
			form.find( '.ajax-poll-item-count' ).hide();
			form.find( '.ajax-poll-item-perc' ).hide();
